	 ...
	 <form class="modal-content"  method="POST" action="">
		...
		<hr></hr>
			...
			try{	
				...
				$stmt = $db->prepare('SELECT * FROM emporoi WHERE usernameE=:usernameE');
				...
				if(isset($_POST['signupbtn'])){
					...
					$stmt = $db->prepare('UPDATE emporoi SET Email_Emporou=:Email_Emporou,OnomaE=:OnomaE,EpithetoE=:EpithetoE,usernameE=:usernameE,passwordE=:passwordE,T_Dieuthunsh_E=:T_Dieuthunsh_E,ThlefwnoE=:ThlefwnoE
										WHERE Email_Emporou=:Email_Emporou');
					$Email_Emporou=$_POST["email"];
					$OnomaE=$_POST["firstname"];
					$EpithetoE=$_POST["lastname"];
					$usernameE=$_POST["username"];
					//$passwordP=$_POST["psw"];
					$T_Dieuthunsh_E=$_POST['dieuthynsh'];
					$ThlefwnoE=$_POST['phone'];
										
					$stmt->bindParam(':Email_Emporou',$Email_Emporou);
					$stmt->bindParam(':OnomaE',$OnomaE);
					$stmt->bindParam(':EpithetoE',$EpithetoE);
					$stmt->bindParam(':usernameE',$usernameE);
					$stmt->bindParam(':passwordE',$passwordE);
					$stmt->bindParam(':ThlefwnoE',$ThlefwnoE);
					$stmt->bindParam(':T_Dieuthunsh_E',$T_Dieuthunsh_E);
			...
			$db=null;
			  ...
			  <input type="text" placeholder="Δώστε νέο ονόμα" value="<?php echo $row['OnomaE'];?> " name="firstname" required>
			  
			  <label for="laststname"><b>Επώνυμο</b></label>
			  <input type="text" placeholder="Δώστε νέο επιθέτο" value="<?php echo $row['EpithetoE']; ?>" name="lastname" required>
			  
			  <label for="dieuthynsh"><b>Διεύθυνση</b></label>
			  <input type="text" placeholder="Δώστε νέα Διεύθυνση" value="<?php echo $row['T_Dieuthunsh_E'];?>" name="dieuthynsh" required>
			  
			  <label for="phone"><b>Τηλέφωνο</b></label>
			  <input type="text" placeholder="Δώστε νέο αριθμό τηλεφώνου" value="<?php echo $row['ThlefwnoE'];?>" name="phone" maxlength="10" required>
			  </br>
			  <label for="email"><b>Email</b></label>
			  <input type="text" placeholder="Δώστε νέο email" value="<?php echo $row['Email_Emporou'];?>" name="email" required>
			  
			  <label for="email"><b>Username</b></label>
			  <input type="text" placeholder="Δώστε νέο username" value="<?php echo $row['usernameE'];?>" name="username" required>
